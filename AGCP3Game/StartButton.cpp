...
void StartButton::Init(DeviceManager* dManager, std::string texPath, DirectX::SimpleMath::Vector2 position, DirectX::SimpleMath::Vector2 scale, DirectX::SimpleMath::Vector2 size, RECT rect)
{
	mPos = position;
	mScale = scale;
	mSize = size;
...
void StartButton::Render(DeviceManager* dManager)
{
...
void StartButton::OnClicked(ResourceManager* rManager, DeviceManager* dManager)
{
	rManager->SetCurrentMap(0);
	rManager->LoadGame(dManager);
	rManager->GetCurrentMap()->SetCurrentZoneNum(0);
}

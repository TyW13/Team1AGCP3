...
void ExitButton::Init(DeviceManager* dManager, std::string texPath, DirectX::SimpleMath::Vector2 position, DirectX::SimpleMath::Vector2 scale, DirectX::SimpleMath::Vector2 size, RECT rect)
{
	mPos = position;
	mScale = scale;
	mSize = size;
...
void ExitButton::Render(DeviceManager* dManager)
{
...
void ExitButton::OnClicked(ResourceManager* rManger, DeviceManager* dManager)
{
	exit(EXIT_SUCCESS);
}
